...
public class NewMainMenu_ConsoleMenu : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (PlayerPrefs.GetInt("TipShown_MainMenuControls", 0) == 0)
        {
            ...
            GonerMenu.Instance.ShowTip($"按<color=yellow>{PlayerInput.Instance.Key_Confirm}</color>確定, 按<color=yellow>{PlayerInput.Instance.Key_Cancel}</color>返回, 使用<color=yellow>方向鍵</color>選擇條目。");
        }
    ...
    private void CheckUpdateScreenText()
    {
        ...
        string text = "<b>>請選擇一個條目</b>\r\n\n";
        string[] array = new string[7] { "模擬猜想", "查看日誌", "遊戲設置", "製作組", "額外內容", "Discord", "退出遊戲" };
        string[] array2 = new string[8] { "模擬猜想", "查看日誌", "進入賭博", "遊戲設置", "製作組", "額外內容", "Discord", "退出遊戲" };
        if (UnlockedGambling)
        {
