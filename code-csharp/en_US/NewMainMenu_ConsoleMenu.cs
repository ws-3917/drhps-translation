...
public class NewMainMenu_ConsoleMenu : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (PlayerPrefs.GetInt("TipShown_MainMenuControls", 0) == 0)
        {
            ...
            GonerMenu.Instance.ShowTip($"<color=yellow>{PlayerInput.Instance.Key_Confirm}</color> to Select, <color=yellow>{PlayerInput.Instance.Key_Cancel}</color> to Return, <color=yellow>Arrow Keys</color> to Navigate");
        }
    ...
    private void CheckUpdateScreenText()
    {
        ...
        string text = "";
        text = "<b>>Select a menu</b>\r\n\n";
        string[] array = new string[7] { "Hypotheticals", "Inspect Logs", "Settings", "Credits", "Extras", "Discord", "Quit" };
        string[] array2 = new string[8] { "Hypotheticals", "Inspect Logs", "Gambling", "Settings", "Credits", "Extras", "Discord", "Quit" };
        if (UnlockedGambling)
        {
